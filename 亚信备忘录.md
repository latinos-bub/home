...
| jdbc:oracle:thin:@(DESCRIPTION=(ADDRESS_LIST=(ADDRESS = <br/>(PROTOCOL = TCP)(HOST = 10.252.9.105)(PORT = 1521)))(CONNECT_DATA =(SERVICE_NAME =JMDM))) | js_zzfx | Gd!ds358   | 21个地市库     |      |

#### 直连地市数据库

sqlplus js_zzfx/Gd\!ds358@DGDM
...
## 查看自助分析模型
 ...
 ```sql
-- 查询 模型 的步骤：1）不知道 模型编码 2）已知模型编码
-- 1）不知道模型编码时,先由浏览器上的 地市名，从 数据库表中 模糊查找可能的 表的id
SELECT * FROM LKG_QUERY_DB_CONFIG WHERE DB_NAME LIKE '%DG%';
-- 然后 根据 上一步获取的 db_id 和 模型名称 模糊查询 宽表
SELECT * FROM LKG_QUERY_TABLE WHERE DB_ID = '467' AND OPER_NAME LIKE '%用户状态翻译表%';
-- 然后就可以直连sql数据库或者 利用客户端连接数据库了

-- 2）若是 已知模型编码（比如，是从 /data_temp/SA... 日志上查看到的）
-- 先根据 模型编码 查询 宽表，获取 db_id = 1505725931615
SELECT * FROM LKG_QUERY_TABLE WHERE OPER_ID = '1545982593863';
-- 然后根据 查询到的 db_id ，去 数据库配置表里面 查询具体是哪一个库的
SELECT * FROM LKG_QUERY_DB_CONFIG WHERE DB_ID = '1505725931615';
-- 如果查询到的 db_type 不是 oracle 的，则不予理会，因为 21地市都是 oracle 的
-- 若是 oracle 库的，那么查看 db_name，然后直连该库，执行sql即可
 ```
